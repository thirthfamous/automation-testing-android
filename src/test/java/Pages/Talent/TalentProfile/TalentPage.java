...
public class TalentPage extends HomePage {
    ...
    public void waitForFirstTalentDevelopmentPlanArrayMyTeamChanged(final String expected){
        new WebDriverWait(this.driver, 30)
                .until(new ExpectedCondition<Boolean>() {
                    @NullableDecl
                    public Boolean apply(@NullableDecl WebDriver webDriver) {
                        return webDriver.findElement(developmentPlanNameFirstArrayMyTeam).getText().equals(expected);
                    }
                });
    }
